...
public class Interface {
    ...
    private static void createUserInterface() throws IOException, SQLException {
        ...
        if (manageUser.checkNoUserInDb(login, phone)) {
            work_logic.createUser(login, password, address, phone);
    ...
    private static void createAccount(User user) throws IOException, SQLException {
        ...
        if (manageAccount.checkNoAccCurrency(user.getId(), currency)) {
            System.out.println("Сделать этот счет основным (y/n)?");
            boolean defAcc = reader.readLine().toLowerCase().equals("y");
            work_logic.createAccount(user.getId(), currency, defAcc);
    ...
    private static void addMoneyOnAccount(User user) throws SQLException, IOException {
        List<Account> accList = manageAccount.selectAccountUsers(user.getId());
        if (accList.isEmpty()) {
        ...
        work_logic.addMoneyBalance(sum, cur, accList.get(account));
    }

    // Показ операций пользователя
    private static void browseOperations(User user) throws SQLException {
        List<Operation> operations = manageOperation.selectAllOperationUser(user);
        System.out.println("Операции пользователя: ");
        int i = 1;
        for (Operation operation : operations) {
            System.out.println(i + ".");
            System.out.println("Дата: " + operation.getDateOfOperation());
            System.out.println("Кому: " + manageAccount.getUserOfAccount(operation.getAccOut()).getPhone());
    ...
    private static void transferMoneyInterface(User user) throws IOException, SQLException, ParseException {
        ...
        String phone = reader.readLine();
        List<User> users = manageUser.selectUserForPhone(phone);
        if (users.isEmpty()) {
        ...
        String message = work_logic.transferMoney(user, users.get(0), accCode, amount);
    ...
    private static void inputIntoSystem() throws IOException, SQLException, ParseException {
        ...
        System.out.print("Введите пароль: ");
        String password = reader.readLine();
        User user = manageUser.getUserForLoginOrPhone(loginOrPhone, password);
        if (user == null) {
